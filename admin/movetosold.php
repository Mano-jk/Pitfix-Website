...
include('db.php');
 $link=mysql_connect("localhost","root","") or die("Cannot Connect to the database!");
  mysql_select_db("pitfix",$link) or die ("Cannot select the database!");
	 $query="INSERT INTO sold(`pid`, `name`, `address`, `e-mail`, `product`, `discription`, `product_weight`, `purchase_date`, `details`) SELECT `pid`, `name`, `address`, `e-mail`, `product`, `discription`, `product_weight`, `purchase_date`, `details` FROM `orders` WHERE pid='$id';";
	 ...
	 ?>
	 	<?php
		$query1="DELETE FROM orders WHERE orderid='$id';";
